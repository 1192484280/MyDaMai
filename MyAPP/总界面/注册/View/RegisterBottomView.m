...
#pragma mark -
#pragma mark - 懒加载
- (HXCTextField *)nameTextField
{
    if (!_nameTextField) {
        _nameTextField = [[HXCTextField alloc] init];
...
- (UIButton *)messageBtn
{
    ...
    if (!_messageBtn) {
        ...
        NSString *title = [NSString stringWithFormat:@"%@协议",[infoDictionary objectForKey:@"CFBundleDisplayName"]];
...
- (UILabel *)messageLabel
{
    if (!_messageLabel) {
        ...
        _messageLabel.text = @"点击'完成'即表示你已阅读并同意";
