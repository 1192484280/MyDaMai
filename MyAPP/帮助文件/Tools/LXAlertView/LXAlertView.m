...
#pragma mark - 带文本的Alert
-(instancetype)initWithText:(NSString *)title cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle textBlock:(LXAlertTextFiledBlock)block{
    if(self=[super init]){
        self.frame=MainScreenRect;
        self.backgroundColor=[UIColor colorWithWhite:.3 alpha:.7];
        ...
        CGFloat messageLabSpace = 40;
        _text=[[UITextField alloc] init];
        _text.backgroundColor=[UIColor whiteColor];
        _text.borderStyle = UITextBorderStyleRoundedRect;
        
        _text.textColor=[UIColor blackColor];
        _text.font=LXADMessageFont;
        
        _text.textAlignment=NSTextAlignmentCenter;
        [_text becomeFirstResponder];
        
        _text.frame=CGRectMake(messageLabSpace, CGRectGetMaxY(_titleLab.frame)+10, AlertView_W-messageLabSpace*2, 35);
        
        
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, 0, AlertView_W, _text.frame.size.height+LXATitle_H+LXABtn_H+40);
        _alertView.center=self.center;
        [self addSubview:_alertView];
        [_alertView addSubview:_titleLab];
        [_alertView addSubview:_text];
...
#pragma mark - 性别选择Alert
-(instancetype)initWithGender:(NSString *)title andMsg: (NSString *)one andMsg: (NSString *)two cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle textBlock:(LXAlertGenderBlock)block{
    ...
    if(self=[super init]){
        ...
        [btnOne setImage:[UIImage imageNamed:@"kong"] forState:UIControlStateNormal];
        [btnOne setImage:[UIImage imageNamed:@"gou"] forState:UIControlStateSelected];
        btnOne.titleLabel.font = [UIFont systemFontOfSize:14];
        btnOne.titleEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 0);
        btnOne.width = ScreenWidth/2;
        btnOne.height = 50;
        btnOne.x = ScreenWidth/2;
        btnOne.y = 0;
        [btnOne addTarget:self action:@selector(onBtnOne:) forControlEvents:UIControlEventTouchUpInside];
        
        btnOne.frame = CGRectMake(messageLabSpace, CGRectGetMaxY(_titleLab.frame)+10, 100, 30);
        self.btnOne = btnOne;
        ...
        [btnTwo setImage:[UIImage imageNamed:@"kong"] forState:UIControlStateNormal];
        [btnTwo setImage:[UIImage imageNamed:@"gou"] forState:UIControlStateSelected];
        btnTwo.titleLabel.font = [UIFont systemFontOfSize:14];
        btnTwo.titleEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 0);
        btnTwo.width = ScreenWidth/2;
        btnTwo.height = 50;
        btnTwo.x = ScreenWidth/2;
        btnTwo.y = 0;
        [btnTwo addTarget:self action:@selector(onBtnTwo:) forControlEvents:UIControlEventTouchUpInside];
        
        btnTwo.frame = CGRectMake(CGRectGetMaxX(btnOne.frame), CGRectGetMaxY(_titleLab.frame)+10, 100, 30);
        self.btnTwo = btnTwo;
        
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, 0, AlertView_W, btnOne.frame.size.height+LXATitle_H+LXABtn_H+40);
        _alertView.center=self.center;
        [self addSubview:_alertView];
        [_alertView addSubview:_titleLab];
        [_alertView addSubview:btnOne];
        [_alertView addSubview:btnTwo];
        ...
        if (cancelTitle) {
            ...
            _cancelBtn.layer.cornerRadius=3;
            _cancelBtn.layer.masksToBounds=YES;
            [_cancelBtn addTarget:self action:@selector(onGenderBtn) forControlEvents:UIControlEventTouchUpInside];
            [_alertView addSubview:_cancelBtn];
        }
        ...
        if (otherBtnTitle) {
            ...
            _otherBtn.layer.masksToBounds=YES;
            [_otherBtn setBackgroundImage:[UIImage imageWithColor:SFQRedColor] forState:UIControlStateNormal];
            [_otherBtn addTarget:self action:@selector(onGenderBtn) forControlEvents:UIControlEventTouchUpInside];
            [_alertView addSubview:_otherBtn];
        }
